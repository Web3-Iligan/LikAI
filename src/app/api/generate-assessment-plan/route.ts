...
const biosecurityTaskSchema = z.object({
  ...
  category: z
    .string()
    .describe(
      "Category of the task (e.g., Infrastructure, Access Control, Water Management)"
    ),
  estimatedCost: z
    .string()
    .describe(
      "Estimated cost range for the task (e.g., '₱500-1,000', '₱0 (existing equipment)')"
    ),
  timeframe: z
    .string()
    .describe(
      "Estimated timeframe for completion (e.g., 'Today', 'Next 7 days', 'Daily')"
    ),
  adaptationReason: z
    .string()
    .optional()
    .describe("Reason why this task is adapted or recommended by AI"),
});
      ...
      const fallbackPlan = [
        {
          ...
          description:
            "Perform a basic inspection of pond dykes for any visible damage or erosion.",
          priority: "critical",
        ...
        },
        {
          ...
          description:
            "Set up a simple footbath at farm entry using local materials.",
          priority: "medium",
