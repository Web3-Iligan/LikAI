...
export function useApi() {
  ...
  useEffect(() => {
    const initActor = async () => {
      ...
      try {
        ...
        const canisterId = process.env.CANISTER_ID_API;
        
        if (!canisterId) {
          throw new Error('API canister ID not found. Please check your environment variables.');
        }
        ...
        setError(err instanceof Error ? err : new Error('Failed to create API actor'));
      }
    };
  ...
  const getWhoAmI = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to call whoami()'));
      return null;
    } finally {
      setLoading(false);
    }
