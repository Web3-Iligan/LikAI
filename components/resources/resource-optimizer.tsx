...
export default function ResourceOptimizer() {
  ...
  const improvementAreas = [
    { id: "pond-water", label: "Pond & Water Care", icon: Droplets, color: "text-blue-600" },
    { id: "access-control", label: "Farm Access Control", icon: Shield, color: "text-emerald-600" },
    { id: "feed-management", label: "Feed Management", icon: UtensilsCrossed, color: "text-amber-600" },
    { id: "disease-readiness", label: "Disease Readiness", icon: Target, color: "text-violet-600" },
    { id: "infrastructure", label: "Infrastructure", icon: Zap, color: "text-cyan-600" },
  ];
  ...
  const generateInvestmentPlan = async () => {
    ...
    const mockRecommendations: Recommendation[] = [
      {
        ...
        benefit: "Prevent disease outbreaks and reduce feed waste through early water quality alerts.",
      ...
      },
      {
        ...
        benefit: "Improve oxygen levels and reduce electricity costs with renewable energy.",
        categoryColor: "bg-blue-50 border-blue-200 text-blue-700",
        icon: Droplets,
      ...
      },
      {
        ...
        benefit: "Reduce feed waste and optimize growth rates through consistent feeding.",
      ...
      },
      {
        ...
        estimatedCost: 4200,
        projectedROI: 450,
        likaiTip: "Build with locally-sourced materials and automatic spray system",
        benefit: "Prevent disease introduction and meet biosecurity standards effectively.",
      ...
      },
      {
        ...
        estimatedCost: 12000,
        projectedROI: 200,
        likaiTip: "Hybrid solar-diesel generator for reliability and efficiency",
        benefit: "Protect your investment during power outages and severe weather events.",
    ...
    const maxBudget = input.budget === "500000+" ? 500000 : parseInt(budgetRange[1] || budgetRange[0]);
    ...
    const totalInvestment = selectedRecommendations.reduce((sum, rec) => sum + rec.estimatedCost, 0);
    const projectedAnnualSavings = selectedRecommendations.reduce((sum, rec) => sum + (rec.estimatedCost * rec.projectedROI / 100), 0);
    const averageROI = selectedRecommendations.length > 0 
      ? Math.round(selectedRecommendations.reduce((sum, rec) => sum + rec.projectedROI, 0) / selectedRecommendations.length)
      : 0;
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-base">
            Tell us your budget and timeline, and LikAI's AI will suggest the best 
            biosecurity upgrades for your farm, showing you how much you can save!
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <Select value={input.budget} onValueChange={(value) => setInput(prev => ({ ...prev, budget: value }))}>
              <SelectTrigger className="h-12">
                <SelectValue placeholder="e.g., Php 50,000 - 200,000" className="text-gray-400" />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-3">
            ...
            <Select value={input.timeline} onValueChange={(value) => setInput(prev => ({ ...prev, timeline: value }))}>
              <SelectTrigger className="h-12">
                <SelectValue placeholder="e.g., Next 6 months" className="text-gray-400" />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-4">
            <label className="text-sm font-medium text-gray-900">
              What areas do you want to improve? <span className="text-red-500">*</span>
            </label>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:gap-6">
              {improvementAreas.map((area) => {
                const Icon = area.icon;
                return (
                  <div key={area.id} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50 transition-colors">
                    <Checkbox
                      id={area.id}
                    ...
                    <label
                      htmlFor={area.id}
                      className="flex items-center gap-3 text-sm font-medium cursor-pointer flex-1 select-none"
                    >
          ...
          <Button
            ...
            disabled={!input.budget || !input.timeline || input.improvementAreas.length === 0}
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12 space-y-4">
          <div className="relative">
            ...
            <Loader2 className="h-6 w-6 text-orange-600 animate-spin absolute -top-1 -right-1" />
          </div>
  ...
  if (viewState === "results" && investmentPlan) {
    const selectedBudgetLabel = budgetOptions.find(b => b.value === input.budget)?.label;
    const selectedTimelineLabel = timelineOptions.find(t => t.value === input.timeline)?.label;
    ...
    return (
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="flex flex-wrap gap-4 text-sm">
              <span><strong>Budget:</strong> {selectedBudgetLabel}</span>
              <span><strong>Timeline:</strong> {selectedTimelineLabel}</span>
            </CardDescription>
          </CardHeader>
        </Card>
        ...
        {/* Recommendations */}
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-gray-900">Recommended Upgrades</h3>
              ...
              <Card key={recommendation.id} className="transition-shadow hover:shadow-md">
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    {/* Icon */}
                    <div className={`flex h-12 w-12 items-center justify-center rounded-lg border ${recommendation.categoryColor}`}>
                    ...
                    <div className="flex-1 space-y-3">
                      <div>
                        ...
                        <Badge className={`mt-1 ${recommendation.categoryColor} border`}>
                      ...
                      <Button 
                        variant="outline" 
                        size="sm" 
                        className="mt-3 bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100 hover:border-blue-300 font-medium"
                      >
